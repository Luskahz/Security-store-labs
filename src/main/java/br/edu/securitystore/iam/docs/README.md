...
Endpoints: `/auth/login`, `/auth/refresh`, `/auth/logout`, `/auth/me`, `/auth/me/sessions` e `/admin/users`, `/admin/roles`, `/admin/permissions` com subrecursos descritos nos controllers. `/api/iam/register` e `/api/iam/me` permanecem por compatibilidade. A autenticação HTTP Basic também permanece para os clientes antigos; as operações de sessão exigem Bearer.
