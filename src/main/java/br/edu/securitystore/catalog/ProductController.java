...
@RestController @RequestMapping("/api/products") public class ProductController {
 ...
 @PostMapping public ResponseEntity<Product> create(@Valid @RequestBody ProductRequest r){return ResponseEntity.status(HttpStatus.CREATED).body(products.save(new Product(r.name(),r.description(),r.price(),r.stock())));}
